...
//
//  Chnages made here results into the text specification and limits the file picker to .jpeg and .pdf files only - by sagar
//

const Signature = () => {
  const dispatch = useDispatch();
  const {signature, enabled} = useSelector((state)=> state.signature);
  const [imagePreview, setImagePreview] = useState(signature);
  ...
  return (
    <div className='w-full'>
       <div className='flex space-x-3 items-center dark:bg-gray-800 dark:border-gray-700'>
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Signature</h2>
       ...
       <div className="flex items-center p-2">
        <div className="w-1/3 border border-gray-300 rounded-lg p-4 h-32 flex items-center justify-center">
            {imagePreview ? (
            <img src={imagePreview} alt="Signature Preview" className="rounded-lg w-full h-full object-contain" />
            ) : (
